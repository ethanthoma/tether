...
func (output *shadowOutput) Write(data []byte) (int, error) {
	if output.buffer.Len()+len(data) > len(shadowPrefix)+shadowTableSize+1 {
		return 0, fmt.Errorf("shadow: evaluator output exceeds protocol limit")
	}
	return output.buffer.Write(data)
}

func readShadowTable(path string) ([]byte, error) {
	if !filepath.IsAbs(path) {
		return nil, fmt.Errorf("shadow: TETHER_BEND_SHADOW must name an absolute executable path")
	}
	...
	if len(raw) != len(shadowPrefix)+shadowTableSize+1 || !strings.HasPrefix(raw, shadowPrefix) || !strings.HasSuffix(raw, "\n") {
		return nil, fmt.Errorf("shadow: invalid evaluator protocol")
	}
	table := raw[len(shadowPrefix) : len(raw)-1]
	if strings.Trim(table, "01") != "" {
...
func productionBendEligibility(store *Store, cfg *Config, nl *nudgeLog, now time.Time) map[*Thread]bool {
	...
	table, err := readShadowTable(cfg.BendShadow)
	if err != nil {
