...
bun "$bend_main" "$experiment_dir/delivery_shadow.bend" -o "$trial_dir/tether-bend-delivery"

(
	cd "$experiment_dir/../.."
	TETHER_BEND_DELIVERY="$trial_dir/tether-bend-delivery" TETHER_BEND_DISPATCH="$trial_dir/tether-bend-dispatch" TETHER_BEND_SHADOW="$trial_dir/tether-bend-shadow" BEND_MAIN="$bend_main" \
		go test -mod=vendor -tags bend -run '^TestBend(FollowUp|Dispatch|Delivery|NativeShadow|NativeDispatch|NativeDelivery)Agreement$' -count=1 .
)
